...
export const AllPhotosModal = ({ open, photos, closeModal }: Props) => {
  ...
  return (
    <>
      <Drawer open={open} onOpenChange={() => !openLightBox && closeModal?.()}>
        <DrawerContent className="h-[90vh]">
          <ScrollArea className="h-[90vh]">
            <div className="container">
              <h5 className="font-medium text-lg">All photos</h5>
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {photos.map(({ src, id }: Photo, index: number) => {
                  ...
                  return (
                    <Image
                      ...
                      width={1000}
                      height={1000}
                      className="aspect-video object-cover cursor-pointer rounded-md"
