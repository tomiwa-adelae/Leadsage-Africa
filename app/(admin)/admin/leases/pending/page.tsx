...
const page = async ({ searchParams }: Props) => {
  ...
  const leases = await getPendingLeases({ query, page, limit: DEFAULT_LIMIT });

  return (
    <div>
      <SiteHeader />
      <div className="py-4 md:py-6 px-4 lg:px-6">
        <h1 className="text-3xl md:text-4xl font-semibold">Pending Leases</h1>
        <p className="text-muted-foreground text-base mt-2.5">
          View and manage all pending leases
        </p>
        <div className="mt-4 space-y-6">
          ...
          {leases.leases.length === 0 && (
            <EmptyState
              title={"No leases"}
              description={
                "There are no pending leases at this moment! They would appear here once you do"
              }
            />
          )}
          {leases.leases.length !== 0 && (
